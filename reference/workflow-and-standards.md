...
**收尾自检（交付前必做）**：① 搜本次改动的文件里 `catch` / `if err != nil` / 提前 `return` / `default:`，逐条确认都有日志；② 确认**没有空 `catch`**、没有吞异常后静默返回；③ 高频回调里的日志有防刷屏处理；④ 日志前缀统一（服务端模块名、客户端 `[模块名]`）。
...
> **为什么单列**：一个项目交上来时**代码全绿、文档全错**，错法都是"改了 A 没改 B"，而且**都不报错**所以没人发现：`策划/验收表.md` 的汇总写「通过 35 行 / 不一致 7 行」而表体实际是 39 / 3；验收表的证据格指向 `tools/<某自检宿主>/` 而该宿主**已搬到 `.ai-tmp/hosts/`**（**证据链断裂 = 等于没有证据**）；`docs/交接-下一棒.md` 说「验收表一行都还没填」而实际早已填满（下一棒照它做就白做）；源码注释引用 `docs/agents/agent-NN-*.md` 而该文件已不存在。
> ⇒ 必须**当成检查项**，不能靠"我记得改过了"。
...
| 4 | **无交接类文档** | `Get-ChildItem -Recurse -Filter *.md \| Where-Object { $_.Name -match '交接\|进度\|NEXT\|下一棒' }` | **0 命中** |
| 5 | **项目文档与全局规则层不冲突** | 对 `tools/ai-skill/` 与 `docs/` grep `以本项目为准\|优先于全局\|优先于通用` | 命中处**逐条判加严还是放宽**；**放宽的一律改掉** |

> **两条推论**：① **搬目录 = 一次全局改名** —— 任何目录 / 文件改名后必须对全项目（含文档、注释、验收表证据格）grep 旧名，**命中归零**才算搬完；② **"过程文档"也有验收责任** —— `docs/` 冻结前必须与现状一致，**过期文档比没有文档更糟**。
...
> **交付前自检**：对 `tools/ai-skill/` 与 `docs/` grep 差异词，命中处逐条判加严还是放宽，**放宽的一律改掉**。
...
> **分类判据（一句话，用来决定"放哪"）**：`判据资产` = **删掉之后还能不能重新判定同一件事**？
> **能重新判定** ⇒ 它是**判据资产** ⇒ **进仓库**（`tools/probes/`，**必须提交**）；**不能** ⇒ 它是**一次性产物** ⇒ 只落 `.ai-tmp/`（用完删）。
> ⚠️ 反面教材：把 281 KB 的探针 + Play 驱动按"一次性产物"删掉 ⇒ **复验能力归零**，最后只能从回收站捞回来。位置表：
...
| `<项目根>/tools/probes/` | **判据资产（必须提交！）**：能复跑的可执行判据 —— 探针 `.cs`、Play 驱动 `.ps1`、量法脚本。判据一句话：**「删掉之后还能不能重新判定同一件事？」不能 ⇒ 它就不是一次性产物**，⛔ 不许留在 `.ai-tmp/`（那里被 gitignore，删了就没了） |
| `策划/` | 策划文档、验收表、素材调研 |
