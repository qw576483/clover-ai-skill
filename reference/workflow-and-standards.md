...
**✅ 白名单（上表三条对它们不成立）**：`clover-doc` 的
[游戏 Demo 清单](https://github.com/qw576483/clover-doc/blob/main/ai/game-demo.md) 里列出的 `clover-project-*`
—— 它们是**引擎 + skill 交付出来的成品**，**允许读、允许对照**：看它**是怎么做成的**
（目录组织、`策划/验收表.md` 的交付口径、`Assets/Editor/*.cs` 生成器的组织方式）。
边界只有一条：**取做法与形状，不取领域内容** —— 它的游戏专有名词 / 模块名 / 数值 / 素材不许搬进本项目。

**允许抄的只有三处**：① **本项目自己**的 skill 与源码；② 引擎源码 / `clover-doc` /
本 skill 的 `patterns`、`scaffold`、`experience`（只有**通用形状**，示例代码不得当 API 用）；
③ 上面白名单里的成品工程（同样只给**做法与形状**）。

**新项目的一切从 0 自己设计**：目录、模块、面板、工程生成器、配表、素材来源全部自己定，
新引入的设施在交付说明里标注"**本项目新增**"。**另一个例外**：用户在**当前这一轮**明确点名
...
> 项目文档**只能加严**；冲突时**照全局做，并把冲突那一行改掉**（`SKILL.md` 的「通用约束」第 4 条）。
> （代价：这句曾被读成"冲突时以项目为准"，于是子 agent 照项目文档把探针堆进 `client/_dev/` ——
> 违反全局 `reference/rules-full.md` 的「临时测试文件规范」 却"有据可依"，还抓不出来。）
...
│   └── 验收表.md                    # ★ 交付闸门（§2）+ 「允许的差异」小节（`reference/rules-full.md` 的「收尾机械自检」）
├── 原版资源/                        # ★ 下载/解包的原版素材唯一来源（`reference/rules-full.md` 的「原版资源」目录；不进 git）
...
> **为什么单列**：一个项目交上来时，**代码全绿、文档全错**，而且错法都是"改了 A 没改 B"：
>
> | 实测现象 | 性质 |
> |---|---|
...
| 4 | **无交接类文档** | `Get-ChildItem -Recurse -Filter *.md \| Where-Object { $_.Name -match '交接\|进度\|NEXT\|TODO-|下一棒' }` | **0 命中**（除项目已交付后的历史冻结；见 `SKILL.md` 的「派活清单」.5 第 8 条） |
| 5 | **项目文档与全局规则层不冲突** | 对 `tools/ai-skill/` 与 `docs/` grep `以本项目为准\|优先于全局\|优先于通用\|与全局 skill 的差异` | 命中处**逐条判加严还是放宽**；**放宽的一律改掉**（`SKILL.md` 的「通用约束」第 4 条） |
...
> 规则原文在 `SKILL.md` 的「通用约束」第 4 条`。本节是它的展开与自检方法。
>
> **为什么单列这条（根因在 skill 不在 agent）**：
...
> 规则原文在 `SKILL.md` 的「写码清单」第 5 条`。本节是它的展开与位置表。
>
...
2. ⛔ **不许**散落在：**项目根**、`tools/`、`client/_dev/`、`Assets/`、`策划/`。
   （违规形态：探针在 `client/_dev/`、脚本在项目根、验证程序在 `tools/xxxcheck/` 各来一份。）
...
| `<项目根>/tools/probes/` | **判据资产（必须提交！）**：能复跑的可执行判据 —— 探针 `.cs`、Play 驱动 `.ps1`、量法脚本（`measure_*.py` 等）。判据一句话：**「删掉之后，还能不能重新判定同一件事？」⇒ 不能 ⇒ 它就不是一次性产物**，⛔ 不许留在 `.ai-tmp/`（那里被 gitignore，删了就没了）。某项目按"一次性产物"把 281 KB 的 `probe.cs` 连同驱动一起删掉 ⇒ 全部复验能力归零，只能从回收站捞回来。 |
   | `策划/` | 策划文档、验收表、素材调研 |
...
交付前删除并写进 `.gitignore`。
⛔ 不许把取证截图放进 `client/Assets/**`。某项目 197 张证据图落在 `Assets/Screenshots/`（6.4 MB），
