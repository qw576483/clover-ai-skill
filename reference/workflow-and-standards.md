...
上表"用户侧三处"指的是 **本项目自己的** skill / 文档 / 源码。工作区里往往还并排躺着别的交付工程（自带 `client/`、`tools/ai-skill/`、`策划/`、`docs/`、`Assets/Editor/*.cs` 生成器、素材目录…），这些都可以对照：**目录组织、分层与模块写法、面板与配表做法、生成器组织、交付口径**（[游戏 Demo 清单](https://github.com/qw576483/clover-doc/blob/main/ai/game-demo.md) 列出的成品最值得看）。

参考做法与形状；本项目自己的规格以**本项目的参考物**为准（`§0` 铁律 2 / 4）。
**新项目的一切自己定**（目录、模块、面板、生成器、配表、素材来源），新引入的设施在交付说明里标注"**本项目新增**"。
