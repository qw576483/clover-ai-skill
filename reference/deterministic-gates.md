...
| 行为种类 | 手段 |
|---|---|
| **必须始终成立的硬性不变量** | **确定性闸门 / 脚本 / hook（**执行**零失败率）** |
...
> ⚠️ **精度修正（⛔ 别把上面那句读成"闸门 = 零失败"）**：
> 两条出处说的是**执行**层面的确定性 —— hook 注册了就一定会被调用、脚本跑了就一定跑。
> **但闸门整体不是零失败率**，它还有三种失败模式：
> ① **假绿**（判据取错对象：grep 源码字符串 ≠ 画面上渲染的文本）；
> ② **假红**（判据取错范围：量到屏幕顶部 HUD 而不是底部那行）；
> ③ **迎合**（判据判"结果"不判"过程" ⇒ 靠改一个数字就能变绿）。
> ⇒ 闸门把失败从「模型的概率性失败」换成「人的确定性错误（判据写错）」—— **非常划算，但不是零**。
> **这三种失败怎么补 → `reference/anti-gaming.md`**（判过程不判结果 · 证据锚点 · L0~L3 分级 · 闸门自检）。
...
> **触发条件是「批次边界」**（`SKILL.md` 的「取证清单」 第 ③ 拍），不是每次 Edit/Write ——
...
- **`Stop`（响应结束时的"收尾动作"）是否能"拒绝结束"，CodeBuddy 官方文档未写明**；
  ...
  每次保存都跑 diff = 把分钟级动作重复 N 次（`SKILL.md` 的「取证清单」）。
