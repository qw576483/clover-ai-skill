...
> **为什么这份文件这么短**：验收 = **用户的抱怨消失 + 关键路径跑过一次 + 表现类看一眼**（`SKILL.md` §4 第 0 条）。
> 曾经的版本有 47 项检查、每种证据都要五列表格 + 哈希 + 时间戳、每条判据都要配负控 —— 那是**验证仪式**，不是验证。
> **用户不会报一个没问题的 bug**；出问题直接去查就行。
> ⇒ 本模板只留 **5 条**"光看会骗人、所以必须机械化"的判据；**其余检查一律可选自检**，红了只当提示，⛔ 不许拿它拦交付。

## 只保留的 5 条（其余都是可选自检）

| # | 常见实现名（可选，⛔ 非要求） | 原则 | 为什么必须机械化 |
|---|---|---|---|
| 1 | `build-compiles` | 编译 / 构建通过 | 编不过，别的都白说 |
| 2 | `evidence-freshness` | 截图 / 证据**新鲜度**（图必须晚于它验证的源码 / 资产） | 人眼看图**永远**看不出这是三天前的 —— 唯一的"反自欺"判据 |
| 3 | `screenshot-refs` | **引用可达**（文档里引的文件 / 行**在盘上**存在） | 悬空引用 = 等于没证据；grep 一下就知道，不靠记性 |
| 4 | `tmp-budget` | **交付卫生**（`.ai-tmp` 体积 / 文件数、无 `bin/obj`、无 `-bak-`、无散落临时文件） | 这条真失控过：单项目 3372 文件 / 990 MB |
| 5 | `verify-script-crash` | **脚本 / AI 自己别崩**（`.ps1` 语法 0 错 + 非 ASCII 必带 BOM） | 闸门自己崩 ⇒ **本次结论作废**，⛔ 不许假装绿 |

**已删除、不再检查**（写清"不检了，为什么"）：
覆盖矩阵 / 覆盖命中率 / 清单自洽 / 汇总核对 / 派活台账列数 / 差异登记逐条核对 /
行数或文件大小的阈值判断（除 `.ai-tmp` 预算这一条真失控过的）/ **"每个判据必须配负控"这类仪式** /
每种证据都要五列表格 + 哈希 + 时间戳。
理由：**这些抓到的都是人眼 / 逻辑一眼能看出来的东西**，或者它们上次红的时候抓的是**自己写错了**。
...
3. **逐行读输出**：`PASS` / `FAIL` / `HUMAN-ONLY`。出现任何 `FAIL` ⇒ ⛔ 不许说"完成 / 交付 / 实测通过"。
   `HUMAN-ONLY` = **只能由人过目**的项（不许执行者代签），例如"这条线歪没歪"。
4. 用户**只跑这一次**，不陪执行者逐点调参。

## ⛔ 写这个脚本时的五个硬坑（避开它们 = 第 5 条闸门的一半）
...
1. **`.ps1` 编码 —— 唯一口径：只看一个事实「文件里有没有非 ASCII 字节」。**
   ...
   ⛔ **不许再有第二套口径**（例如"代码行必须 ASCII、注释可以不带 BOM"）：两套并存时，**连"备份一个含中文的 `.ps1`"这个动作本身都会把闸门刷红**（实测 cs16 `tools/verify.ps1`：138 个非 ASCII 字节 + 无 BOM ⇒ 只能存成 `.txt` 才敢备份）。
...
> **这是一个"参考清单"，不是要求**（`required` 计数 = **0**）：上面那 **5 条原则**（编译 / 证据新鲜度 / 引用可达 / 交付卫生 / 脚本别崩）是要求；下面 6 个名字只是**常见实现名**，供项目抄用。
