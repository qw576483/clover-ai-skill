...
**⛔ 第 0 步：先定朝向，再谈布局**（见 `SKILL.md` §6「闸门 0」）。**没有"默认横屏"这回事** ——
...
- ⛔ **`preserveAspect` 不是解药**：竖图放进横画布会两侧留黑边，那是"朝向判错了"的症状，
  不是"用 `preserveAspect` 兜住"。原版竖构图**铺满竖屏**才对。
- 判据：交付前把成品与 A 官方截图**同机位并排**，朝向/比例/元素位置逐项 `一致`（→ `SKILL.md` §0.1 ②）。

**布局口径**（朝向定了之后）：以最小可用为原则，⛔ 不依赖外部资源做占位（见下方「占位 UI 规则」）。
