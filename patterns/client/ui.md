...
**⛔ 第 0 步：先定横版/竖版（屏幕方向），再谈布局**（见 `SKILL.md` 的「初始化清单」第 1 条）。**没有"默认横屏"这回事** ——
先查清了再动手，判一次、写进 `策划/策划案/{A}参考规格.md` 顶部、全程照做：

| 横版/竖版 | 画布参考分辨率 | 面板布局口径 |
|---|---|---|
| **横版**（宽 > 高） | `1920×1080` | 左右并排合法；内容框宽可上 1600 |
| **竖版**（高 > 宽） | `1080×1920` | **一律上下排布 + 角锚点**；内容框宽 ≤ ~1000；⛔ 横排三件套（标签–输入框–按钮一行）必须拆成三行 |
...
- 工程侧同时要改 `ProjectSettings.asset`：`defaultScreenOrientation`（竖版 = `1` Portrait）、
  `defaultScreenWidth/Height`（竖版 = `1080×1920`）、`allowedAutorotateTo*`（只留目标屏幕方向）。
- ⛔ **`preserveAspect` 不是解药**：竖图放进横画布会两侧留黑边，那是"横版/竖版判错了"的症状，
  不是"用 `preserveAspect` 兜住"。原版竖构图**铺满竖屏**才对。
- 判据：交付前把成品与 A 官方截图**同机位并排**，横版竖版/比例/元素位置逐项 `一致`（→ `SKILL.md` 的「交付清单」第 3 条）。

**布局口径**（横版/竖版定了之后）：以最小可用为原则，⛔ 不依赖外部资源做占位（见下方「占位 UI 规则」）。
