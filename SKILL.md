...
- **⛔ 派活前先给用户一句预告（治"看起来卡死"）**：`Task` 是**同步**调用 —— 发出去之后主 agent 就在**等它返回**，**这段时间界面上不会有任何新输出**（几分钟到几十分钟），用户会以为卡死而打断（实测：`code=10003 This operation was aborted` / `No result found` **每次都紧跟在用户发消息之后**；失败与时长无关 —— 70 分钟的成功返回过，4 分钟的被打断过）。
  ⇒ 派活前用一行说清**四件事**：要派什么 / **预计多久** / **期间界面不动属正常，不是卡死** / **你打断也不会丢工作**（子 agent 仍在后台跑、产出照落盘，主 agent 会**先查盘再派接力棒**）。
- ⛔ **单片规模上限（防"等待期被打断"）**：一片 = **一个交付单元**（能被一条命令判生死的那一级）+ **≤ 约 20 分钟 / ≤ 130 轮工具调用**；超了就**拆片**，⛔ 不许"一片干到底"。
  ...
  ⇒ 配套两条（治"**卡死几小时才发现**"）：**① 任务书必须逐字要求"每条命令显式超时 + 心跳文件 + 计时自检（>12 分钟立刻落盘回报）"**（模板 = `patterns/multi-agent.md` 第 3 步 **9b** / `scaffold/agent-impl.md` **§2.5**）；
  **② 主 agent 的止损手段 = 用户随时打断**（打断**无损**：子 agent 后台继续跑、产出照落盘、主 agent 查盘后**只派接力棒补没做完的**）⇒ **看到 15 分钟没动静就打断**，⛔ 不要等几小时来确认。
